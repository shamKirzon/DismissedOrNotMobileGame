...
public class ExitDoor : MonoBehaviour
{
    ...
    public string nextSceneName = "Level2_Library";
    ...
    public void UnlockDoor()
    {
        if (isLocked)
        {
            isLocked = false;
            SetUnlockedAppearance();
            Debug.Log("🔓 EXIT DOOR UNLOCKED! You can now escape!");

            // Show dialogue when door unlocks
            if (DialogueManager.Instance != null)
            {
                DialogueManager.Instance.ShowDialogue("That must've unlocked the door... Finally I can leave!");
            }
        }
        else
        {
    ...
    void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("Player") && !isTransitioning)
        {
            if (isLocked)
            {
                Debug.Log("The door is locked! Collect all keycards first.");
            }
            else
            {
